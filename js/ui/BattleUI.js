...
class BattleUI {
     ...
     */
    showBattleMessages(messages, callback) {
    ...
    backToPreviousMenu() {
        if (this.menuStack.length > 0) {
            ...
            safePlaySound(getSoundID('CANCEL'));
        }
    }
    ...
    moveSelection(direction) {
        ...
        if (newIndex !== oldIndex) {
            ...
            safePlaySound(getSoundID('CURSOR_MOVE'));
        }
    }
    ...
    confirmSelection() {
        ...
        safePlaySound(getSoundID('CONFIRM'));
    ...
    handleSkillSelection(item) {
        const eventBus = getEventBus();
        if (item.pp <= 0) {
            safePlaySound(getSoundID('ERROR'));
...
// 辅助函数 - 从 window 获取 UIColor
function getUIColor(colorName) {
    return window.UIColors?.[colorName] || '#ffffff';
}

// 辅助函数 - 从 window 获取 ElementType
function getElementType(typeName) {
    return window.ElementType?.[typeName] || 'normal';
}

// 辅助函数 - 从 window 获取 MonsterTemplates
function getMonsterTemplates() {
    return window.MonsterTemplates || {};
}
