...
type DialogRegistry = Record<string, DialogNode>;
type NPCDialogRegistry = Record<string, string[]>;
type QuestTemplateRegistry = Record<string, QuestTemplate>;
 ...
 * 格式: {dialogId}: {对话数据}
 */
var DialogNodes: DialogRegistry = {
    // ========== 新手村NPC对话 ==========
    ...
    'mayor_greeting': {
        id: 'mayor_greeting',
        npcId: 'npc_mayor',
        condition: (gameState) => !Boolean(gameState.flags.received_starter),
        lines: [
        ...
        choices: [
            {
                text: '（使用默认名字）',
                nextDialog: 'mayor_receive_monster',
                action: { type: 'give_selected_starter' }
            }
        ]
    },
    ...
    'mayor_post_starter': {
        ...
        npcId: 'npc_mayor',
        condition: (gameState) => Boolean(gameState.flags.received_starter) && !Boolean(gameState.flags.quest_main_02_completed),
        lines: [
            { speaker: '村长', text: '你已经选好了自己的第一只怪兽，不能再重新选择了。', portrait: 'normal' },
    ...
    'mayor_quest_complete': {
        ...
        npcId: 'npc_mayor',
        condition: (gameState) => Boolean(gameState.flags.quest_main_02_completed) && Boolean(gameState.flags.quest_main_03_completed) && !Boolean(gameState.flags.quest_main_04_completed),
        lines: [
    ...
    'mayor_daily': {
        id: 'mayor_daily',
        npcId: 'npc_mayor',
        condition: (gameState) => Boolean(gameState.flags.received_starter) && Boolean(gameState.flags.quest_main_04_completed),
        lines: [
    ...
    'shopkeeper_greeting': {
        ...
        condition: (gameState) => !Boolean(gameState.flags.talked_to_shopkeeper),
        lines: [
    ...
    'granny_greeting': {
        id: 'granny_greeting',
        npcId: 'npc_granny',
        condition: (gameState) => !Boolean(gameState.flags.quest_side_01_started) && !Boolean(gameState.flags.quest_side_01_completed),
        lines: [
    ...
    'granny_complete': {
        ...
        condition: (gameState) => {
            ...
            return Boolean(quest && quest.status === 'ready_to_complete');
        },
        lines: [
    ...
    'granny_daily': {
        id: 'granny_daily',
        npcId: 'npc_granny',
        condition: (gameState) => Boolean(gameState.flags.quest_side_01_completed),
        lines: [
    ...
    'boy_greeting': {
        id: 'boy_greeting',
        npcId: 'npc_boy',
        condition: (gameState) => !Boolean(gameState.flags.talked_to_boy),
        lines: [
    ...
    'boy_daily': {
        id: 'boy_daily',
        npcId: 'npc_boy',
        condition: (gameState) => Boolean(gameState.flags.talked_to_boy) && !Boolean(gameState.flags.received_starter),
        lines: [
    ...
    'girl_greeting': {
        id: 'girl_greeting',
        npcId: 'npc_girl',
        condition: (gameState) => !Boolean(gameState.flags.quest_side_02_started) && !Boolean(gameState.flags.quest_side_02_completed) && Boolean(gameState.flags.received_starter),
        lines: [
    ...
    'girl_in_progress': {
        id: 'girl_in_progress',
        npcId: 'npc_girl',
        condition: (gameState) => Boolean(gameState.flags.quest_side_02_started) && !Boolean(gameState.flags.delivered_letter),
        lines: [
    ...
    'girl_complete': {
        id: 'girl_complete',
        npcId: 'npc_girl',
        condition: (gameState) => Boolean(gameState.flags.delivered_letter),
        lines: [
    ...
    'girl_daily': {
        id: 'girl_daily',
        npcId: 'npc_girl',
        condition: (gameState) => Boolean(gameState.flags.quest_side_02_completed),
        lines: [
    ...
    'researcher_greeting': {
        ...
        condition: (gameState) => !Boolean(gameState.flags.talked_to_researcher),
        lines: [
    ...
    'researcher_receive_letter': {
        ...
        condition: (gameState) => {
            ...
            return Boolean(gameState.flags.quest_side_02_started) && Boolean(hasLetter);
        },
        lines: [
 ...
 * 根据 NPC ID 获取可用的对话列表
 */
var NPCDialogMap: NPCDialogRegistry = {
    // 新手村NPC
    'npc_01': [
        'welcome'
    ],
...
var QuestTemplates: QuestTemplateRegistry = {
    // ========== 主线任务 ==========
 ...
 */
function getDialogForNPC(npcId: string, gameState: GameStateData): string | null {
 ...
 * @returns {Object} 任务实例
 */
function createQuestInstance(questId: string): ActiveQuest | null {
    ...
    return {
        ...
        rewards: {
            ...
            items: template.rewards.items.map(item => ({ ...item })),
...
// 兼容性别名 - 用于测试
var DialogData: DialogRegistry = DialogNodes;
