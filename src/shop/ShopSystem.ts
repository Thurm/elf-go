 ...
 */
type RuntimeShopInventoryItem = ShopInventoryItem & {
    currentStock: number;
...
class ShopSystem {
    ...
    currentShopInventory: RuntimeShopInventoryItem[];
    isOpen: boolean;
    ...
    sellItem(inventoryUid, quantity = 1) {
        ...
        // 检查是否可以出售
        const priceConfig = itemTemplate.price;
        const sellPrice = typeof priceConfig === 'number' ? priceConfig : priceConfig.sell;

        if (sellPrice === null || itemTemplate.type === ItemType.KEY_ITEM) {
            eventBus.emit(GameEvents.UI_NOTIFICATION, {
                type: 'error',
                message: '该物品无法出售！'
            });
